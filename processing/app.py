...
def populate_stats():
    ...
    result = get_latest_stats()  # [Stats, 200]
    ...
    for event in events:
        if event['item_price'] > result.max_buy_price:
            result.max_buy_price = event['item_price']
        result.num_buys += 1
    ...
    for event in events:
        if event['item_price'] > result.max_sell_price:
            result.max_sell_price = event['item_price']
        result.num_sells += 1
